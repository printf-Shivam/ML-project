...
st.markdown("""
    <h1 style='text-align:center; color:#0D7377;'>
        🛡️ ML Based Malware Detection System
    </h1>
...
with st.sidebar:
    ...
    - Shivam Kumar Singh
    """)
    st.markdown("**Section:** 23412G2 | **Sem:** 6th")
...
with tab1:
    ...
    if uploaded:
        # Save to temp file so feature extractor can read it
        ...
        for i, (name, model) in enumerate(models_to_run):
            ...
            label = "🦠 Malware Detected" if pred == 1 \
                    else "✅ Benign File"
            conf  = prob * 100 if pred == 1 \
                    else (1 - prob) * 100
            ...
            with cols_display[i]:
                ...
                ax.barh(0, 100 - conf,
                        left=conf, color="#e0e0e0", height=0.4)
                ax.set_xlim(0, 100)
                ax.axis("off")
        ...
        st.markdown("---")
        st.markdown("""
        > ⚠️ **Note:** This system performs **static analysis only**.
        > It reads the file's PE structure without executing it.
        > For obfuscated or packed malware, dynamic analysis
...
# ════════════════════════════════════════════════════════════
with tab2:
    st.markdown("### Model Performance on PE Malware Dataset")
    ...
    with col2:
        ...
        ax.bar(x - width/2, rf_vals,  width,
               label="Random Forest", color="#1B2A4A")
        ax.bar(x + width/2, xgb_vals, width,
               label="XGBoost",       color="#0D7377")
        ax.set_ylim(98.5, 100.1)
        ax.set_xticks(x)
        ax.set_xticklabels(metrics)
        ax.set_ylabel("Score (%)")
        ax.legend()
        ax.set_title("RF vs XGBoost — PE Dataset")
        ax.grid(axis="y", alpha=0.3)
        st.pyplot(fig, use_container_width=True)
        plt.close()

    st.markdown("---")
    st.markdown("### Model Performance on EMBER 2018 v2 Dataset")
    st.caption("20,000 samples | 2,381 features | Internal + External test")
...
with tab3:
    ...
    This system was developed as a Major Project for
    **CSE 3968 — Machine Learning Concept 2** at
    **ITER, Siksha 'O' Anusandhan Deemed to be University**.

    #### What it does
    - Classifies Windows `.exe` files as **Malware or Benign**
      using static PE (Portable Executable) feature analysis
    - Provides a **risk level** (Low / Medium / High / Critical)
      based on malware probability
    - Compares two ML models: **Random Forest** and **XGBoost**

    #### Datasets Used
    | Dataset | Samples | Features | Purpose |
    |---|---|---|---|
    | EMBER 2018 v2 | 20,000 | 2,381 | Algorithm benchmarking |
    | dataset_malwares.csv | 19,611 | 78 | Real .exe prediction |
    ...
    #### Team
    Aman Kumar · Harsh Kumar · Puja Kumari · Shivam Kumar Singh
    **Section 23412G2 | 6th Semester | CSE**
    """)